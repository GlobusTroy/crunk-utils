...
- DraggableNodeBase - Abstract base class that defines draggable signals and interface
-  - Defines required signals: drag_started, drag_ended, dropped
-  - Signals include the parent/control node that is being dragged
-  - Abstract methods for starting/ending drag operations
-  - Register with the DragDropSystem on _ready
- ControlDraggableNode - Control-based implementation of DraggableNodeBase
-  - @export draggableNode : Control = get_parent()
-  -  - Throw warning or error if draggableNode is not a Control node
-  - Use parent control's gui_event signal to handle mouse events
-  - Implements abstract methods from DraggableNodeBase
- ReceptorNodeBase - Abstract base class that defines receptor signals and interface
-  - Defines required signals: receptor_entered, receptor_exited
-  - Signals include the receptor parent/control node
-  - Defines abstract method: can_receive_drop(draggable_node) -> bool
-  - Register with the DragDropSystem on _ready
- ControlReceptorNode - Control-based implementation of ReceptorNodeBase
-  - @export receptorNode : Control = get_parent()
-  -  - Throw warning or error if receptorNode is not a Control node
-  - Use parent control's gui_event signal to handle mouse events
...
-  - Defines the interface for drag drop system strategies and handles connecting to signals
- BasicDragDropSystemStrategy - Basic implementation of the drag drop system strategy with default behaviors
- - Dragged node follows cursor with easing
- - Dragged node snaps to receptor when dropped
- - Dragged node returns to original position when dropped outside of a valid receptor
